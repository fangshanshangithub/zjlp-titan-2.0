...
import com.zjlp.face.titan.common.utils.SparkUtils
  ...
  private def getIdMapFromES = {
    spark.sql(
      s"CREATE TEMPORARY TABLE userIdVIdInES " +
        ...
        s"OPTIONS (resource '${MigrationConfig.get("titan-es-index")}/rel', es.read.metadata 'true')")
...
object DataMigration extends scala.Serializable {
  def main(args: Array[String]) {
    ...
    if (MigrationConfig.get("clean-init-data").toBoolean) {
    ...
    if (MigrationConfig.get("relation-syn").toBoolean) {
      dataMigration.relationsSyn()
    }

    SparkSession.clearActiveSession()

    if (MigrationConfig.get("clean-titan-instances").toBoolean) {
